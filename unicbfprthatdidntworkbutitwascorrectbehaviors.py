...
def create_si_pr_barrier_certificate_centralized(gamma=100, safety_radius=0.3, magnitude_limit=0.2,
                                                 confidence_level=0.9):
    def barrier_certificate(dxu, xp):
        ...
        for i in range(N - 1):
            for j in range(i + 1, N):
                ...
                A[count, (2 * i)] = -2 * (b_x * math.cos(xp[2,i]) + b_y * math.sin(xp[2,i]))
                A[count, (2 * i + 1)] = -2 * 0

                A[count, (2 * j)] = 2 * (b_x * math.cos(xp[2,j]) + b_y * math.sin(xp[2,j]))
                A[count, (2 * j + 1)] = 2 * 0
        ...
        dxu[:1, idxs_to_normalize] = dxu[:1, idxs_to_normalize] * (magnitude_limit / norms[idxs_to_normalize])

        f_mat = -2 * np.reshape(dxu, 2 * N, order='F')
        f_mat = f_mat.astype('float')
        result = qp(H, matrix(f_mat), matrix(A), matrix(b))['x']
        rt = np.reshape(result, (2, -1), order='F')
        print(rt)
        return rt
...
def transition_model(i, p, dt):
    delta_x = (dxu[0, i]) * dt * math.cos(p[2, i])
    delta_y = (dxu[0, i]) * dt * math.sin(p[2, i])
...
uni_barrier_cert = create_si_pr_barrier_certificate_centralized(safety_radius=safety_radius,
                                                               confidence_level=confidence_level)
