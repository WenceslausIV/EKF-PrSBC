...
publisher137 = rospy.Publisher('/cmd_vel_137', Twist, queue_size=1)
...
def control_callback(event):
    ...
    twist137.linear.x = dxu[0, 1]
    twist137.linear.y = 0.0
    twist137.linear.z = 0.0
    twist137.angular.x = 0
    twist137.angular.y = 0
    twist137.angular.z = dxu[1, 1]
    publisher131.publish(twist131)
    publisher137.publish(twist137)
    # predict(0)
    # measure(0)
    # print(p)
...
def ekf_update_function137(event):
    predict(1)
    measure(1)
...
def central():
    ...
    ekf_timer137 = rospy.Timer(rospy.Duration(0.05), ekf_update_function137)
