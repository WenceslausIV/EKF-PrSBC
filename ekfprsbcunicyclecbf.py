    ...
    def barrier_certificate(dxu, xp):
        ...
        for i in range(N - 1):
            for j in range(i + 1, N):
                ...
                A[count, (3 * i)] = -2 * b_x  * math.cos(xp[2,i]) * dxu[0,i]
                A[count, (3 * i + 1)] = -2 * b_y * math.sin(xp[2,i]) * dxu[0,i]

                A[count, (3 * j)] = 2 * b_x  * math.cos(xp[2,j]) * dxu[0,j]
                A[count, (3 * j + 1)] = 2 * b_y * math.sin(xp[2,j]) * dxu[0,j]
        ...
        dxu[:, idxs_to_normalize] = dxu[:, idxs_to_normalize] * (magnitude_limit / norms[idxs_to_normalize])

        f_mat = -2 * np.reshape(dxu, 3 * N, order='F')
        f_mat = f_mat.astype('float')
        result = qp(H, matrix(f_mat), matrix(A), matrix(b))['x']

        result = np.reshape(result, (3, -1), order='F')

        for i in range(N):
            id = 0
            if i % 3 == 0:
                v = math.sqrt(result[:,i] ** 2 + result[:,i+1] ** 2)
                
                dxu[:, 2 * id] = v
                
                dxu[:, 2 * id + 1] = result[:, i + 2]
                id = id + 1
