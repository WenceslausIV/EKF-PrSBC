...
goal_points = np.array([[0., 1.], [-1., 0.], [math.pi / 2,  math.pi]])

p = np.array([[0,0],[0,0],[0,0]])

x_rand_span_x = 0 * np.random.randint(1, 2, (1, N))  
...
#([[mean1, mean2],[std1, std2]])   ([[mean1, mean2],[std1, std2]])
robotGaussianDistInfox = np.array([[0, 0],[0, 0]]) #all the robots' x pos distribution info including mean and std
robotGaussianDistInfoy = np.array([[0, 0],[0, 0]]) #all the robots' y pos distribution info including mean and std
...
def callback(data, args):
    ...
    nx = x +  np.random.normal(0, 0.1, x.shape)
...
def dist_callback(data, args):
    robotGaussianDistInfox[0,args] = array_msg.data[0]
    robotGaussianDistInfox[1,args] = array_msg.data[1]
    robotGaussianDistInfoy[0,args] = array_msg.data[2]
    robotGaussianDistInfoy[1,args] = array_msg.data[3]

def control_callback(event):
    global p, dxu, robotGaussianDistInfox, robotGaussianDistInfoy
...
def ekf_update_function(event):
    ...
    robotGaussianDistInfox[1,0] = math.sqrt(cov_list[0][0,0])
    robotGaussianDistInfoy[0,0] = p[1,0]
    robotGaussianDistInfoy[1,0] = math.sqrt(cov_list[0][1,1])
...
def central():
    ...
    #this is for robot 0 so subscribe to robot 1's mean&std
    rospy.Subscriber('mean_std_array_1', Float64MultiArray, dist_callback, 1)
