...
firstFlag = []
for i in range(N):
    cov_fillup = np.full((3, 3), 0.01, dtype=np.float64)

    cov_list.append(cov_fillup)
    firstFlag.append(1)
    ...
    def barrier_certificate(dxi, x):
        ...
        for i in range(N - 1):
            for j in range(i + 1, N):
                ...
                twobytwo_cov = np.array([ [ cov_list[j][0][0], cov_list[j][0][1] ], [ cov_list[j][1][0], cov_list[j][1][1] ] ])
                eigenvalues, eigenvectors = np.linalg.eig(twobytwo_cov)
                ...
                b2_x = -b1_x
                b1_x = b1_x + (p[0, i] - p[0, j])
                b2_x = b2_x + (p[0, i] - p[0, j])
...
def transition_model(i, p, dt):
    ...
    #pcopy for multi thread not entering critical region
    pcopy = p 
...
def find_hphr(i):
    ...
    Q = np.array([[0.1, 0, 0], [0, 0.1, 0], [0.001, 0, 0]])

    h = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    hphr = h @ cov_list[i] @ h.T + Q
    return hphr
...
def predict(i):
    ...
    pcopy = transition_model(i, p, dt)
    cov_list[i] = updateCov(i, pcopy, cov_list, dt)


def measure(i):
    global p, cov_list, nx
    h = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    K = cov_list[i] @ h.T @ np.linalg.inv(find_hphr(i))

    p_i = np.array([[p[0, i]], [p[1, i]], [p[2, i]]])
