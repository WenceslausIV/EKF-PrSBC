...
def callback(data, args):
    ...
    theta = tf_conversions.transformations.euler_from_quaternion(
        [data.pose.orientation.x, data.pose.orientation.y, data.pose.orientation.z, data.pose.orientation.w])[1]
    
    x[0,i] = data.pose.position.z
    x[1,i] = data.pose.position.x
    x[2,i] = theta

    if firstFlag == 1:
        p[0,i] = data.pose.position.z
        p[1,i] = data.pose.position.x
        p[2,i] = theta
        firstFlag = 0
...
def control_callback(event):
    ...
    twist.linear.x = dxu[0,0]/10
    twist.linear.y = 0.0
    twist.linear.z = 0.0
    twist.angular.x = 0
    twist.angular.y = 0
    twist.angular.z = dxu[1,0]/10
