    ...
    def barrier_certificate(dxi, x):
        ...
        for i in range(N - 1):
            for j in range(i + 1, N):
                ...
                # siwon added
                z_value = stats.norm.ppf(confidence_level)  # if confidence level is 95% then z_value is about 1.96
...
def ekf_update_function(event):
    ...
    array_msg.data = [robotGaussianDistInfox[0, 0], robotGaussianDistInfox[1, 0], robotGaussianDistInfoy[0, 0],
                      robotGaussianDistInfoy[1, 0]]
