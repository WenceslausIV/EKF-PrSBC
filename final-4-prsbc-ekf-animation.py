...
firstFlag = []
for i in range(N):
    cov = np.full((3, 3), 0.1, dtype=np.float64)
    cov_list.append(cov)
    cov_list2.append(cov)
    firstFlag.append(1)


def create_clf_unicycle_pose_controller(approach_angle_gain=1, desired_angle_gain=2.7, rotation_error_gain=0.4):
...
def create_si_pr_barrier_certificate_centralized(gamma=100, safety_radius=2, magnitude_limit=0.2,
                                                 confidence_level=0.9):
...
def measure(i):
    ...
    if i == 3:
        ...
        if len(gt_list) <= 1000:
            gt_list.append(x.copy())
            pp_list.append(p.copy())
            np_list.append(nx.copy())
            #cov_per_list0.append(cov_list[0].copy())
            #cov_per_list1.append(cov_list[1].copy())
            #cov_per_list2.append(cov_list[2].copy())
            #cov_per_list3.append(cov_list[3].copy())
            print(len(gt_list))
...
def pos_compare():
    ...
    def update(frame):
        ...
        circle0 = patches.Circle((gt_list[frame][0, 0], gt_list[frame][1, 0]), 0.4, fill=False, color='black')
        ...
        radius1 = circle_1_r
        '''
        circle1 = patches.Circle((gt_list[frame][0, 1], gt_list[frame][1, 1]), 0.4, fill=False, color='black')
        ...
        circle2 = patches.Circle((gt_list[frame][0, 2], gt_list[frame][1, 2]), 0.4, fill=False, color='black')
        ...
        circle3 = patches.Circle((gt_list[frame][0, 3], gt_list[frame][1, 3]), 0.4, fill=False, color='black')
...
def control_callback(event):
    ...
    d = np.sqrt((goal_points[0][:4] - p[0][:4]) ** 2 + (goal_points[1][:4] - p[1][:4]) ** 2)
    if (d < .2).all():
        goal_points = goal_points[:, [1, 0, 3, 2]]
...
def plot_updater(event):
    global x, nx, p, pp_list, gt_list, np_list
...
def central():
    ...
    plot_timer = rospy.Timer(rospy.Duration(0.2), plot_updater)
