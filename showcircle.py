...
dxu = np.zeros((2, N))
dxi = np.zeros((2, N))
goal_points = np.array([[-2.0, 1.0], [0.0, 0.], [math.pi / 2, -math.pi/2]])
...
def trap_cdf_inv(a, c, delta):
    ...
    if abs(A - C) < 1e-5:  # then is triangle
        # assuming sigma > 50
        b1 = (A + C)  # 1 - area_vec[1] should be very close to 0.5
        b2 = -b1
...
def create_si_pr_barrier_certificate_centralized(gamma=100, safety_radius=0.2, magnitude_limit=0.2, confidence_level=0.95):
    ...
    def barrier_certificate(dxi, x):
        ...
        #print('decreasing?', 1.96 * robotGaussianDistInfox[1,0])
        for i in range(N - 1):
            for j in range(i + 1, N):
                ...
                #print('this should decrease', XRandSpan[0,i])
                ...
                h = 0
                if b_x > b_y:
                    h = np.linalg.norm(t1) ** 2 - safety_radius ** 2
                else:
                    h = np.linalg.norm(t4) ** 2 - safety_radius ** 2
...
def updateCov(i,p, dt):
    ...
    m = np.array([[ahh * (dxu[0,i]),0],[0,ahh2*(dxu[1,i])]])
...
def update_cov2(i):
    ...
    Q = np.array([[0.001 ,0,0],[0,0.001,0],[0,0,ahh2**2]])
...
def measure(i):
    ...
    xcovlist.append(cov_list[i][0,0])
...
def pos_compare():
    ...
    ax.set_xlabel('Time')
    ax.set_ylabel('X variance')
    ax.set_title('x variance figure')
    print('*****************************')
    ekf_xl = []
    ekf_yl = []
    ...
    for t in range(400):  # Loop for 100 updates (adjust as needed)
        ...
        print(t)
        ekf_x, ekf_y = 0.1*t, xcovlist[t]  # Replace with EKF data retrieval
        ekf_xl.append(ekf_x)
        ekf_yl.append(ekf_y)
...
def show_circle():
    ...
    ekf_xl = []
    ekf_yl = []
    for t in range(1000):  # Loop for 100 updates (adjust as needed)

        print(t)
        radius = 0.2 + circlelist[t]
        center = (t*0.1,0)
        if t % 3 == 0:
