...
robotGaussianDistInfox = np.array([[p[0,0], x[0,1]],[math.sqrt(cov[0][0,0]), math.sqrt(cov[0][0,0]) ]]) #all the robots' x pos distribution info including mean and std
robotGaussianDistInfoy = np.array([[p[1,0], x[1,1]],[math.sqrt(cov[0][1,1]), math.sqrt(cov[0][1,1]) ]]) #all the robots' y pos distribution info including mean and std
    ...
    def barrier_certificate(dxi, x):
        ...
        for i in range(N - 1):
            for j in range(i + 1, N):
                ...
                #until here
                b2_x, b1_x, sigma = trap_cdf_inv(XRandSpan[0, i], XRandSpan[0, j], p[0, i] - p[0, j])
                b2_y, b1_y, sigma = trap_cdf_inv(XRandSpan[1, i], XRandSpan[1, j], p[1, i] - p[1, j])
...
def control_callback(event):
    global p, dxu
    dxu = uni_controller(p, goal_points)/20
...
def ekf_update_function(event):
    ...
    robotGaussianDistInfox[1,1] = math.sqrt(cov_list[0][0,0])
    robotGaussianDistInfoy[0,1] = x[1,1]
    robotGaussianDistInfoy[1,1] = math.sqrt(cov_list[0][1,1])
